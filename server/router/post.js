...
router.post('/submitpost', async (req, res) => {
    // get info from body
    const { title, message, category, tag, mode, askingCompany, askingYear } = req.body;
    // validation
    if (!title || !message || !category || !tag ||!mode || !askingCompany || !askingYear) {
    ...
    try {
        ...
        if (postExist) {
            ...
            const post = new Post({ title, message, category, tag, mode, askingCompany, askingYear });
